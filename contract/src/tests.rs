...
use crate::contract::types::{BoxRarity, BoxStatus, Probability, Reward};
...
fn test_boxes_for_quest_for_owner_amount() {
    ...
    assert_eq!(
        contract
            .questboxes_for_quest_per_owner(quest.id, user1(), None)
            .len(),
        1
    );
    ...
    assert_eq!(
        contract
            .questboxes_for_quest_per_owner(new_quest_id, user1(), None)
            .len(),
        2
    );
    assert_eq!(
...
fn test_claim_box_status() {
    ...
    let needed_box = quest_boxes_per_owner
        .iter()
        .find(|quest_box| quest_box.quest_id == quest.id && quest_box.box_id == box_data.box_id);

    assert!(needed_box.is_some(), "Questbox should exist");
    assert!(
        needed_box.unwrap().box_status
            == JsonBoxStatus::Claimed {
                reward: JsonReward::Near {
                    amount: ONE_NEAR.into(),
                },
            },
        "Questbox should be claimed"
    );
}

#[test]
fn test_claiming_one_box_doesnt_affect_others() {
    let (mut contract, mut context, quest) = setup(None, None, None);
    ...
    assert_eq!(box_2_data.box_id, box_2_id);
    assert_eq!(box_2_data.box_status, JsonBoxStatus::NonClaimed);
}
...
fn test_users_increases() {
    ...
    assert_eq!(
        contract.get_users(quest.id, pagination.clone()),
        vec![user1(), user2(), user3()]
    );
}
...
fn test_testnet_default_nft_contracts() {
    let (mut contract, mut context, quest) =
        setup(None, Some(testnet_user()), Some(testnet_user()));
...
fn test_mainnet_default_nft_contracts() {
    let (mut contract, mut context, quest) =
        setup(None, Some(mainnet_user()), Some(mainnet_user()));
